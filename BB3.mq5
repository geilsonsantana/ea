...
int previousSignal = 0; // 0: sem sinal, 1: compra, 2: venda
...
bool LoadIndicators(){
    ...
    if (CopyBuffer(FX_Handle2, 0, 1, 1, FX_Value2) != 1) {
        Print("Erro ao obter o valor do indicador FX.ex5 para a vela anterior em TimeFrame2 e CCI_Period2", GetLastError());
        return false;
    }

    CopyBuffer(B3_Handle, 0, 1, 2, buyBuffer);
    CopyBuffer(B3_Handle, 1, 1, 2, selBuffer);
    /*
        if (CopyBuffer(B3_Handle, 0, 1, 2, buyBuffer) != 1) {
            Print("Erro ao obter o valor buyBuffer do indicador B3.ex5.", GetLastError());
            return false;
        }
        if (CopyBuffer(B3_Handle, 1, 1, 2, selBuffer) != 1) {
            Print("Erro ao obter o valor selBuffer do indicador B3.ex5.", GetLastError());
            return false;
        }
    */
    return true;
...
void OnTick()
{
   ...
   // Obtém o sinal do indicador B3.
   double b3Signal = iCustom(Symbol(), PERIOD_CURRENT, "B3", 0, 1);

   // Verifica se os sinais do indicador FX estão alinhados com o sinal do B3.
...
void StringToIntegerArray(string str, long &first, long &second) {
    ...
    if (count == 2) {
        ...
        second = StringToInteger(buffer[1]);
    }
}
